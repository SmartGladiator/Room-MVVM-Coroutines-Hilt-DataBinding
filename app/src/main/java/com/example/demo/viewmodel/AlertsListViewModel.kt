...
constructor(private val mainRepository: MainRepository) : ViewModel() {
    ...
    val postStateFlowList: MutableStateFlow<ApiState> = MutableStateFlow(ApiState.Empty)
    ...
    fun getEmployeeList(mHashMap: HashMap<String, Any>) = viewModelScope.launch {
        postStateFlow.value = ApiState.Loading
        mainRepository.getEmployeeList("")
            .catch { e ->
                postStateFlow.value = ApiState.Failure(e)
            }.collect { data ->
                postStateFlow.value = ApiState.Success(data)
